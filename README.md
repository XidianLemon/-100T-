...
1.动归1、确定dp数组及下标含义，本题dp数组指的是斐波那契数列的每一项，下标含义就是每一项。动归2、确定递推公式，本题递推公式也给出了就是dp[i]=dp[i-1]+dp[i-2]。3、dp数组初始化，dp[0]=0,dp[1]=1。4、确定遍历顺序，从前到后。5、举例推到dp数组，按照递推公式举例，0112358没有问题。

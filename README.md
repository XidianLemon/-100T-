...
## 2.1.删除有序数组中的重复项
这道题和移除元素思路一样，都是快慢指针，但是不同的地方在于，移除元素是移除给定值，因此不存在前后比较，而这道题要比较的重复项，因此需要前后比较，所以初始的话，慢指针在0，快指针在1。

快的如果不等于慢的，我先把慢的走一个，然后再把快的给慢的，这样目的是保留当前的然后下一个赋值新的，因为是有序的，所以如果不等则这个慢的就是第一次出现不等的情况！
